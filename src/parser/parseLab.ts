...
export function parseLab(color: string): [number, number, number, number?] {
    // const labRegex = /^lab\(\s*(\d+(\.\d+)?%?)\s+(-?\d+(\.\d+)?%?)\s+(-?\d+(\.\d+)?%?)(?:\s*\/\s*(none|0|1|0?\.\d+|[0-9]{1,3}%))?\s*\)$/;
    const labRegex = /^lab\(\s*(\d+(\.\d+)?%?|none)\s+(-?\d+(\.\d+)?%?|none)\s+(-?\d+(\.\d+)?%?|none)(?:\s*\/\s*(none|0|1|0?\.\d+|[0-9]{1,3}%))?\s*\)$/;
    ...
    const alpha = match[7] ? (match[7] === 'none' ? 1 : match[7].includes('%') ? parseFloat(match[7]) / 100 : parseFloat(match[7])) : undefined;
