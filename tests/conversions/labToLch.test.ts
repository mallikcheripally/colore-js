...
describe('labToLch', () => {
    test('converts LAB to LCH correctly (string format)', () => {
        ...
        expect(result).toBe('lch(50, 36.05551275463989, 56.309932474020215)');
    });

    test('converts LAB to LCH correctly (object format)', () => {
        const result = labToLch(50, 20, 30, false);
        expect(result).toEqual({ l: 50, c: 36.05551275463989, h: 56.309932474020215 });
    });
    ...
    test('handles edge cases for LAB values (string format)', () => {
        ...
        expect(result3).toBe('lch(50, 180.31361568112376, 135.2246881796245)');
    });
    ...
    test('handles edge cases for LAB values (object format)', () => {
        ...
        expect(result3).toEqual({ l: 50, c: 180.31361568112376, h: 135.2246881796245 });
    });
